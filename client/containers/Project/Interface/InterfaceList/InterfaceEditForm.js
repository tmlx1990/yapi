...
const XML_HEADER = constants.XML_HEADER;
@connect(
    state => {
      return {
...
class InterfaceEditForm extends Component {
  ...
  initState(curdata) {
    ...
    // 增加xml
    if (curdata.req_body_xml && curdata.req_body_xml.length === 0) {
      delete curdata.req_body_xml;
    }
    if (curdata.req_body_form) {
      curdata.req_body_xml = curdata.req_body_xml.map(item => {
        item.type = item.type === 'text' ? 'text' : 'file';
        return item;
      });
    }
    ...
    try {
      this.props.form.validateFields((err, values) => {
        ...
        if (!err) {
          ...
          } else if (values.req_body_type === 'xml') {
            {/* 新增xml保存数据 */}
            values.req_headers
                ? values.req_headers.map(item => {
                  if (item.name === 'Content-Type') {
          ...
          values.req_query = values.req_query
              ...
              : [];
          //XML
          values.req_body_xml = values.req_body_xml
              ? values.req_body_xml.filter(item => item.name !== '')
              : [];
            ...
            <div className="panel-sub">
              ...
              >
                <Col style={{ minHeight: '50px' }}>
                  <Row type="flex" justify="space-around">
                    XML header：
                    <Input type='text' defaultValue={XML_HEADER} style={{ width: '60%' }} />
                  </Row>
                  <EasyDragSort
                      data={() => this.props.form.getFieldValue('req_body_form')}
                      onChange={this.handleDragMove('req_body_form')}
